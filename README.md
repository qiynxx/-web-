...
Web 端点击“新建项目”，填写名称和项目截止日期后，后端会在“硬件项目管理”下创建独立 Base、创建节点表和连线表、建立按任务负责人分列的“人员分工看板”、把项目截止日期、token 与表 ID 登记到“Web项目管理可视化”，随后 Web 自动切换到新项目并打开对应飞书文档。

打开项目后点击“项目设置”，可同步更新 Web 项目标题和项目截止日期、目录表中的对应字段及独立 Base 文档标题；如果目录写入失败，后端会尝试把已改动的 Base 标题回滚到原名称。

项目流程图和甘特图由 Web 根据 Base 中的“项目节点”和“项目连接关系”两张表实时渲染。流程图节点直接显示 DDL；甘特图按日期展开节点计划，开始日期、截止日期和详情面板中的修改会自动写回 Base。Base 本身保存结构化记录，不会自动生成同款 Web 画布；Web 顶部提供“节点表”和“连线表”入口用于直接核对底层数据。
...
测试覆盖独立项目创建与重命名、人员分工看板创建与配置、时间字段自动补齐与 Base 写回、甘特图范围和 DDL 状态计算、重命名失败回滚、CLI 矩阵结果映射、连线关联、节点删除时清理关联连线，以及权限错误传递。
